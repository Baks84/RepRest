...
namespace MegaApi
{
    ...
    public class Mega
    {
        ...
        public IMegaRequest GetQuota(Action<long,long> OnSuccess, Action<int> OnError)
        {
            var filesRequest = new MRequestGetUserQuota<MResponseGetUserQuota>(User);
            filesRequest.Success += (s, a) =>
            {
                if (OnSuccess != null) { OnSuccess(a.storage_max, a.storage_used); }
            };
            filesRequest.Error += (s, e) => 
            { 
                if (OnError != null) { OnError(e.Error); } };
            transport.EnqueueRequest(filesRequest);
            return filesRequest;
        }
        public UserQuota GetQuotaSync()
        {
            UserQuota result = null;
            int? error = null;
            GetQuota((l1, l2) => result = new UserQuota() {Max_Quota=l1, Used_Quota=l2 }, (e) => error = e).ResetEvent.WaitOne();
            if (result == null || error != null)
            {
                throw new MegaApiException((int)error, "Could not get the quota");
            }
            return result;
        }
        ...
        public static MegaUser LoadAccount(string filePath)
        {
            ...
            //}
            //return u;
            return LoadAccount(File.ReadAllBytes(filePath));
        }

        public static MegaUser LoadAccount(byte[] accountData)
        {
            MegaUser u = null;
            try
            {
                var encrypted = accountData;
        ...
        public void SaveAccount(string filePath)
        {
            ...
            File.WriteAllBytes(filePath, SaveAccountData());
        }

        public byte[] SaveAccountData()
        {
            byte[] decrypted;
            if (!string.IsNullOrEmpty(User.Email))
            {
